...
出力フォルダ（`out/`）は、直下に銘柄ごとのフォルダ（`[Ticker]_[CompanyName]`）を作成し、その配下に `market_data/` や `analysis/` のサブフォルダを作成して整理します。
...
* **データ読み込みルール**:
  - 分析やモデル構築を行う際は、必ず `out/[Ticker]_[Company_Name]/market_data/` 配下に格納されたCSV/JSONファイルを一次情報として動的に読み込んで使用します。
* **Gitワークフロー**:
  - 変更はローカルでのコミットに留め、**ユーザーから明確な指示があるまで `git push` は絶対に行いません**。
  - 指示があってプッシュを実行する際は、**指示がない限り `origin` へプッシュを行います**。
