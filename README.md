...
## 7. データおよびファイルの管理ポリシー (File Management Policy)

本プロジェクトで生成・管理される各種ファイルは、効率的な再分析および過去ログの保存（監査トレール）の観点から、以下のポリシーで取り扱われる設計となっています。

### ① 財務データベース・バリュエーションモデル ＝ 【自動上書き更新 (Roll-forward)】
- **対象ファイル:** `prices.csv` などの一次データ、および `dcf_[Ticker].xlsx`, `3statement_[Ticker].xlsx` などの Excel モデル
- **設計:** 常に**同一のファイル名に対して上書き保存**されます。再実行時には最新データが適用され、数式構造やレイアウトを維持したまま数値が最新の状態に自動ロールフォワード更新されます。

### ② 定性レポート・プレゼンテーション資料 ＝ 【日付別で新規作成 (履歴蓄積)】
- **対象ファイル:** `Kioxia_Investment_Thesis_[YYYYMMDD].md` などの Markdown レポート、および `Kioxia_Pitch_[YYYYMMDD].pptx` などの資料
- **設計:** ファイル名に**実行日の日付 (YYYYMMDD)** が自動付与されます。別日に再実行した場合は新しくファイルが生成され、過去のレポートは上書きされずに残るため、分析の変遷（タイムライン履歴）が蓄積されます。
