...
class TestGenerateLBO(unittest.TestCase):
    def setUp(self):
        self.test_dir = "./out/test_lbo_data"
    ...
    def test_build_lbo_model(self):
        from utils import get_latest_financial_data
        ticker_data = get_latest_financial_data(self.ticker_dir, self.ticker)
        
        generate_lbo.build_lbo_model(ticker_data, self.ticker_dir)

        # 保存先パス確認
        out_path = os.path.join(self.ticker_dir, "analysis", "lbo_7203.T.xlsx")
        self.assertTrue(os.path.exists(out_path))

    @patch('sys.argv', ['generate_lbo.py', '7203', '--outdir', './out/test_lbo_data'])
    def test_main(self):
        if hasattr(generate_lbo, 'main'):
            generate_lbo.main()
        
        # 期待される出力ファイルが存在するか確認
        output_file = os.path.join(self.ticker_dir, "analysis", "lbo_7203.T.xlsx")
